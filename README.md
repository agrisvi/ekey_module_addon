...
6. **ekey** appears in the sidebar. No token is needed there; ingress requests are
   already authenticated by Home Assistant.
...
It contains one `config.yaml` and this README. That is the whole add-on as far as Home
Assistant is concerned: with `image:` set the Supervisor pulls the container and never
looks at a `Dockerfile` or a source tree, so shipping them here would serve nobody.
